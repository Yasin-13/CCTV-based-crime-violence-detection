...
while True:
    ...
    if violence_detected and frame_count == violence_start_frame + 30:
        ...
        denoised_frame = cv2.GaussianBlur(equalized_frame, (5, 5), 0)

        # Save the enhanced frame as PNG
        cv2.imwrite('alert_frame.png', denoised_frame, [cv2.IMWRITE_PNG_COMPRESSION, 0])

        # Send the alert with timestamp to the Telegram group
        message = f"Violence detected at {current_time}"
        with open('alert_frame.png', 'rb') as f:
            bot.sendPhoto(telegram_group_id, f, caption=message)
